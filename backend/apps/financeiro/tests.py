...
class FinanceiroOcorrenciasTests(TestCase):
    def setUp(self):
        self.client = APIClient()
        self.user = User.objects.create_user(
            username='fin_ocorrencias',
            password='fin123',
            role_id='2',
            environments=['Financeiro'],
            filiais={'Financeiro': ['Ibiporã', 'Rondonópolis']},
        )
        self.other = User.objects.create_user(
            username='ind_ocorrencias',
            password='ind123',
            role_id='2',
            environments=['Indicadores'],
            filiais={'Indicadores': ['Ibiporã']},
        )

    def test_meta_returns_filiais_and_justificativas(self):
        response = self.client.get(
            '/api/financeiro/ocorrencias/meta/',
            **auth_headers(self.user, 'Financeiro'),
        )
        self.assertEqual(response.status_code, 200)
        self.assertIn('Ibiporã', response.data['filiais'])
        self.assertTrue(len(response.data['justificativas']) >= 4)

    def test_ops_recebidas_crud_paginated_and_audit(self):
        for index in range(12):
            self.client.post(
                '/api/financeiro/ocorrencias/ops-recebidas/',
                {
                    'filial': 'Ibiporã',
                    'contrato': f'C{index:04d}',
                    'dataPagamento': '2026-07-01',
                    'mdfeEncerrado': index % 2 == 0,
                },
                format='json',
                **auth_headers(self.user, 'Financeiro'),
            )
        listed = self.client.get(
            '/api/financeiro/ocorrencias/ops-recebidas/?page=1&page_size=10',
            **auth_headers(self.user, 'Financeiro'),
        )
        self.assertEqual(listed.status_code, 200)
        self.assertEqual(listed.data['count'], 12)
        self.assertEqual(len(listed.data['results']), 10)
        self.assertIn('mdfeEncerrado', listed.data['results'][0])
        ...
        self.assertEqual(AuditLog.objects.count(), before + 1)
        self.assertTrue(AuditLog.objects.filter(action='financeiro.ocorrencia.ops.criado').exists())

        pk = created.data['id']
        patched = self.client.patch(
            f'/api/financeiro/ocorrencias/ops-recebidas/{pk}/',
            {'mdfeEncerrado': True},
            format='json',
            **auth_headers(self.user, 'Financeiro'),
        )
        self.assertEqual(patched.status_code, 200)
        self.assertTrue(patched.data['mdfeEncerrado'])
    ...
    def test_gnre_and_notas_create(self):
        gnre = self.client.post(
            '/api/financeiro/ocorrencias/gnre-icms/',
            {
                'filial': 'Ibiporã',
                'cte': '7573',
                'valorGuia': '1371.74',
                'periodoReferencia': '2025-08',
        ...
        self.assertEqual(gnre.status_code, 201)
        self.assertEqual(gnre.data['cte'], '7573')
        self.assertFalse(gnre.data['validada'])

        nota = self.client.post(
            '/api/financeiro/ocorrencias/notas-pagas/',
            {
                'filial': 'Ibiporã',
                'nfs': '506',
                'fornecedor': 'ROELA MECANICA',
                'valor': '725.00',
                'dataEmissao': '2025-07-24',
                'envioProvisaoLuft': '2025-07-30',
                'dataPagamento': '2025-08-12',
                'justificativa': 'Atraso provisão Luft / alterações cod.',
            },
            format='json',
            **auth_headers(self.user, 'Financeiro'),
        )
        self.assertEqual(nota.status_code, 201)
        self.assertEqual(nota.data['nfs'], '506')

    def test_ocorrencias_forbidden_without_financeiro_module(self):
        response = self.client.get(
            '/api/financeiro/ocorrencias/ops-recebidas/',
            **auth_headers(self.other, 'Indicadores', 'Ibiporã'),
        )
