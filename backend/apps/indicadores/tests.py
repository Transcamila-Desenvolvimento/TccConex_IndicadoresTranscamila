...
class OcorrenciasIndicadoresTests(TestCase):
    def setUp(self):
        self.client = APIClient()
        self.user = User.objects.create_user(
            username='ind_ocorr',
        ...
        from apps.financeiro.models import OpsRecebidaOcorrencia, GnreIcmsOcorrencia
        OpsRecebidaOcorrencia.objects.create(
            filial='Ibiporã',
            contrato='100',
            data_pagamento=date(2025, 8, 1),
            mdfe_encerrado=True,
        )
        OpsRecebidaOcorrencia.objects.create(
            filial='Rondonópolis',
            contrato='101',
            data_pagamento=date(2025, 8, 2),
            mdfe_encerrado=False,
        )
        GnreIcmsOcorrencia.objects.create(
            filial='Barueri',
            cte='7573',
            valor_guia=Decimal('100.00'),
            periodo_referencia='2025-08',
            data_pagamento=date(2025, 8, 1),
            validada=False,
        )
        GnreIcmsOcorrencia.objects.create(
            filial='Ibiporã',
            cte='801',
            valor_guia=Decimal('50.00'),
            periodo_referencia='2025-08',
            data_pagamento=date(2025, 8, 4),
            validada=True,
        )

    def test_ops_indicadores_summary(self):
        response = self.client.get(
            '/api/indicadores/ocorrencias/ops/',
            **auth_headers(self.user, 'Indicadores', 'Ibiporã (Matriz)'),
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['summary']['total'], 2)
        self.assertEqual(response.data['summary']['mdfePendentes'], 1)
        self.assertEqual(response.data['summary']['percentualFalha'], 50.0)
        self.assertEqual(len(response.data['byFilial']), 2)
        self.assertTrue(len(response.data['byPeriod']) >= 1)
        self.assertTrue(len(response.data['byMonth']) >= 1)
        self.assertIn(2025, response.data['meta']['availableYears'])
        self.assertIn('Ibiporã', response.data['meta']['availableFiliais'])
        self.assertIn('Rondonópolis', response.data['meta']['availableFiliais'])

    def test_ops_indicadores_filters_year_months_and_granularity(self):
        headers = auth_headers(self.user, 'Indicadores', 'Ibiporã (Matriz)')
        by_year = self.client.get(
            '/api/indicadores/ocorrencias/ops/',
            {'year': 2025, 'months': '8', 'filial': 'Ibiporã'},
            **headers,
        )
        self.assertEqual(by_year.status_code, 200)
        self.assertEqual(by_year.data['summary']['total'], 1)
        self.assertEqual(by_year.data['meta']['months'], [8])
        self.assertIn('day', by_year.data['meta']['allowedGranularities'])

        by_day = self.client.get(
            '/api/indicadores/ocorrencias/ops/',
            {'year': 2025, 'months': '8', 'granularity': 'day'},
            **headers,
        )
        self.assertEqual(by_day.status_code, 200)
        self.assertEqual(by_day.data['meta']['granularity'], 'day')
        self.assertFalse(by_day.data['meta']['granularityAdjusted'])
        periods = [row['period'] for row in by_day.data['byPeriod']]
        self.assertIn('2025-08-01', periods)
        self.assertIn('2025-08-02', periods)

        # Sem um único mês: visão diária volta para mensal.
        by_day_all_months = self.client.get(
            '/api/indicadores/ocorrencias/ops/',
            {'year': 2025, 'granularity': 'day'},
            **headers,
        )
        self.assertEqual(by_day_all_months.status_code, 200)
        self.assertEqual(by_day_all_months.data['meta']['granularity'], 'month')
        self.assertTrue(by_day_all_months.data['meta']['granularityAdjusted'])
        self.assertNotIn('day', by_day_all_months.data['meta']['allowedGranularities'])

    def test_gnre_indicadores_summary(self):
        response = self.client.get(
            '/api/indicadores/ocorrencias/gnre/',
            **auth_headers(self.user, 'Indicadores', 'Ibiporã (Matriz)'),
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['summary']['total'], 2)
        self.assertEqual(response.data['summary']['naoValidadas'], 1)
        self.assertEqual(float(response.data['summary']['valorTotal']), 150.0)
        self.assertEqual(response.data['summary']['percentualFalha'], 50.0)
        self.assertTrue(len(response.data['byPeriod']) >= 1)
        self.assertIn('Barueri', response.data['meta']['availableFiliais'])

        # Mesma sistemática de OPs: dia só quando pedido explicitamente com 1 mês.
        by_month = self.client.get(
            '/api/indicadores/ocorrencias/gnre/',
            {'year': 2025, 'months': '8', 'granularity': 'month'},
            **auth_headers(self.user, 'Indicadores', 'Ibiporã (Matriz)'),
        )
        self.assertEqual(by_month.status_code, 200)
        self.assertEqual(by_month.data['meta']['granularity'], 'month')
        ...
        self.assertEqual(by_day.status_code, 200)
        self.assertEqual(by_day.data['meta']['granularity'], 'day')
        self.assertIn('valorTotal', by_day.data['byPeriod'][0])

        guias = self.client.get(
            '/api/indicadores/ocorrencias/gnre/guias/',
            {'year': 2025, 'search': '7573', 'page': 1, 'pageSize': 10},
            **auth_headers(self.user, 'Indicadores', 'Ibiporã (Matriz)'),
        )
        self.assertEqual(guias.status_code, 200)
        self.assertEqual(guias.data['count'], 1)
        self.assertEqual(guias.data['results'][0]['cte'], '7573')
